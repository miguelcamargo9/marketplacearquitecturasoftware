...
public class busquedaProveedorServlet extends HttpServlet {
    @EJB
    private UsuariosFacade miPerfilSession;
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Usuarios> misPerf = miPerfilSession.findAll();
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet busquedaProveedorServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<center>");
            out.println("<TABLE>");
            out.println("<TR>");
            out.println("<TH>Proveedores</TH>");
            out.println("<TH>Correo</TH>");
            out.println("</TR>");
            for(Usuarios user: misPerf){
                out.println("<TR>");
                if(user.getIdPerfil()==3){
                    out.println("<TD>"+user.getNombreComercial()+"</TD>");
